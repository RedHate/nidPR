...
unsigned char *mipsRegisterArray[]={"zero", "at", "v0", "v1", "a0", "a1", "a2", "a3", "a4", "a5", "a6", "a7", "t4", "t5", "t6", "t7", "s0", "s1", "s2", "s3", "s4", "s5", "s6", "s7", "t8", "t9", "k0", "k1", "gp", "sp", "fp", "ra"};
#else
...
void mipsDecode(unsigned int a_opcode){
	...
	switch((a_opcode & 0xFC000000) >> 26){
		...
		case 0x13:
			pspDebugScreenPuts("cop1x     "); /*there is an inherent flaw in the channeling system of mips imm, we need channels 1 2 3 (if this instruction exists on psp)*/
			mipsImm(a_opcode, 0, 0);
	...
	switch((a_opcode & 0xFF000000) >> 24){
		...
		case 0x43:
			pspDebugScreenPuts("cop0     "); //broken due to mips imm (i dont know what i meant anymore)
			mipsImm(a_opcode, 1, 0); 
		break;  
